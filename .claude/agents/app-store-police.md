...
- **Bare-name shellouts**: `subprocess.run(["ffmpeg", …])` (yours *or a dependency's* — e.g. `mlx_whisper`, `faster_whisper`) resolves `ffmpeg` on `PATH`, which under the sandbox excludes Homebrew → `FileNotFoundError`, silent empty output. Bundle the binary and prepend its dir to `PATH` at package-import time — before any submodule that shells out is imported.
- **System-file reads that raise `PermissionError`**: Python 3.12+ `mimetypes.init()` reads `/etc/apache2/mime.types` etc.; under the sandbox that raises and poisons the module → HTTP 500 on every static asset. Neutralise before first use (`mimetypes.knownfiles = []`, at package-import time).
...
Full worked case study (a real Python-sidecar app's first-upload ASC rejections, plus the config gates and build-pipeline traps) is in the appendix.
...
> ⚠️ **`codesign --verify --strict` passing ≠ App Store Connect will accept the upload.** Local tools validate *signature validity*; ASC's server-side pipeline also enforces *App Store policy* that no local command reproduces. One real app's first TestFlight upload passed every local `codesign`/`spctl` check and was still bounced by three server-side rejections (missing category, nested binaries without app-sandbox, an unsigned framework Mach-O). When you clear a `.app` in post-build mode, say explicitly which findings are *local-tool-verifiable* and which are *ASC-policy-only* (below) — the second class you can only reason about, not prove, until the upload runs. See the known-rejections ledger at the end.
...
- **Build must be arch-consistent with what it actually ships.** If a nested binary is single-arch (say, an arm64-only sidecar), the whole Release must exclude the other arch (`EXCLUDED_ARCHS = x86_64`) — a fat host wrapping a thin sidecar is an upload-time arch-mismatch.
...
- `LSApplicationCategoryType` is set to a valid category string — **not optional for App Store: a missing category is an outright upload rejection**, not a metadata nicety (a real first TF upload was bounced for exactly this; `public.app-category.productivity` fixed it). Set it in the build settings (`INFOPLIST_KEY_LSApplicationCategoryType`), not just App Store Connect.
...
- *For a typical Mac pipeline app:* most named SDKs on Apple's list are iOS-oriented, so what you mainly need is your own app-level manifest covering the required-reason APIs you (or your bundled libs) actually call.
...
>
> _One shipping implementation does this with a freeze-time strip in the PyInstaller
> spec plus a build gate that greps the frozen sidecar for App-Store-prohibited
> strings; verified present-and-now-stripped 16 Jul 2026._
...
- A local-server-plus-webview app is generally fine here, but the reviewer will look.
...
### §4.8 — Login Services (connecting a third-party work account)

**The false positive to NOT raise.** An app that lets a user connect their own
Microsoft or Google work account to fetch *their own* files is **not** a §4.8
trigger, and flagging it as a missing "Sign in with Apple" is wrong. 4.8 governs
a third-party login used to *set up or authenticate the user's primary account
with your app* — and it carries an explicit exception for client apps where
users sign in to a third-party service directly to reach their own content.

Two independent limbs of that exception apply to an app that has **no account
system of its own** — no app account, no server, no sign-in it controls — where
the OAuth grant reaches the user's own files and nothing else. Check whether
both hold before raising anything here; if they do, do not raise it.

### §5.1.1 / §5.1.2 — ingress is not egress, and that is the whole distinction

**Keep these two straight. They are orthogonal, and conflating them produces bad
findings in both directions.**

- **§5.1.2(i) — data going OUT.** You must clearly disclose where personal data
  will be shared with third parties, *including third-party AI*, and obtain
  explicit permission first. Content sent to a cloud LLM for processing is
  squarely this. Explicit consent genuinely is required, and a consent surface
  plus a version log is how it is obtained.
- **Importing the user's own files shares nothing.** A download *from* the
  user's own account *to* their own machine transmits nothing to anyone; the
  outbound traffic is their sign-in and a query for their own file list. The
  "explicit permission" limb is not engaged, and the consent screen they see is
  the vendor's own, listing scopes in the vendor's words — a more specific
  disclosure than the app's would be.

**Consequently: do not demand that an import feature be added to an AI-consent
surface, and do not treat its absence there as a stale consent version.** The
two surfaces answer different questions. Beware the coincidence that trips this
up — the same vendor may supply both an LLM API key and an unrelated OAuth
client for file access. Different consoles, different credentials, different
data flows. Where a user gets their own files from is their business.

Nor is a claim like *"your recordings stay on your device"* falsified by an
import feature: importing puts files **on** the device, and nothing uploads them.

**What §5.1.1 DOES require:** clear disclosure of the third-party data source
and what happens to the data after — what is accessed, how, retention, and how
to revoke. That belongs in the privacy policy at the canonical URL every vendor
console points at, naming the vendors, the read-only scoping, credential
storage, and the revocation path (Entra ▸ Enterprise Applications, Google
Account ▸ Security). Check that the policy page is current before submission —
and check the *policy*, not a documentation page that merely discusses privacy.
...
- Apple's three official patterns: (1) use `Process` / Swift only, (2) bundle a minimal Python via `PythonKit` / embedded framework, (3) ship the interpreter as a signed helper binary via the "sidecar" pattern. Pattern 3 is the usual choice for a Python sidecar.
...
**A real app's first-upload rejections (14 Jul 2026), for pattern-matching future runs** — all three passed local `codesign --verify` and were caught only by ASC's server-side validation:
1. **Missing `LSApplicationCategoryType`** → added `public.app-category.productivity`.
2. **Nested executables (sidecar, ffmpeg, ffprobe) had no `app-sandbox` entitlement** → added `app-sandbox`+`inherit` to the sidecar; authored a minimal ffmpeg entitlements file and wired it into the ffmpeg signing script.
...
Ground truth from getting a sandboxed macOS app — a Swift host wrapping a PyInstaller
Python sidecar plus bundled ffmpeg/ffprobe — through App Store Connect. Pattern-match
against this before asserting a build is clean; every item below was a *real* rejection
or gate failure, dated for provenance.
...
- **Xcode Run Script phases don't reliably inherit your shell env**: a skip-flag env var (a `*_SKIP_SIDECAR_ENSURE`-style skip flag) never reached an in-archive phase, so its guard aborted the archive. Gate such phases on a *guaranteed* build setting (`CONFIGURATION`), not a propagated env var.
